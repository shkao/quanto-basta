...
def compare_execution_times(num_iterations=5):
    ...
    print(
        f"Cached function average time: {cache_total_time/num_iterations:.6f} seconds"
    )
    print(f"Uncached function total time: {uncache_total_time:.6f} seconds")
    print(
        f"Uncached function average time: {uncache_total_time/num_iterations:.6f} seconds"
    )
    print(
        f"Speed improvement: {uncache_total_time/cache_total_time:.2f}x faster with caching"
    )
